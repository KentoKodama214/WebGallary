...
`currentPassword` の照合はアカウントが取得できない場合でも BCrypt 照合を1回行い、応答時間を一定に保つ
（`accountNo` は常に呼び出し元自身のセッション由来のため他人の列挙には使えないが、
...
流す増幅経路になるため）。間引き処理（全走査＋ソート）は多数スレッド同時流入時も一定間隔に1回に集約し、
並べ替えキーは各エントリのモニタ下で1度だけ写し取ってからソートする（並行更新による比較の揺れを防ぐ）。
間引きが追いつかない間は新規アカウントのエントリ追加を見送り、エントリ数を上限付近で頭打ちにする。
