...
public class PhotoController {
   ...
   * 写真一覧の写真取得
   *
   * <p>リクエストの抽出条件に該当する写真を、指定の並び順で取得する
   *
   ...
   * 写真登録上限チェック
   *
   * <p>指定のアカウントが写真の登録枚数の上限に達しているかをチェックする
   *
   * @param photoAccountId ページ所有者のアカウントID
   ...
   * @return {@link PhotoEditResponse}
   * @throws GalleryException 以下のいずれかに該当する場合 ・リクエストパラメータが不正の場合 ・写真の所有者以外がリクエストした場合 ・写真の削除に失敗した場合
   */
  ...
  public ResponseEntity<PhotoEditResponse> deletePhoto(
    ...
    if (result.hasErrors()) {
      log.info(
          "Invalid input. (PhotoNo: {}, ImageFilePath: {})",
          photoDeleteRequest.getPhotoNo(),
          photoDeleteRequest.getImageFilePath());
      throw ErrorEnum.INVALID_INPUT.toException();
    }
